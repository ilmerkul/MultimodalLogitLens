...
class HookedTransformer(HookedRootModule):
    ...
    def __init__(
        ...
        if tokenizer is not None:
            ...
            if self.cfg.tokenizer_name in NON_HF_HOSTED_MODEL_NAMES:
                ...
                # not official implementation
                if "llava" in self.cfg.tokenizer_name.lower():
                    ...
                    self.set_tokenizer(self.processor.tokenizer,
    ...
    def input_to_embed(
        ...
        # not official implementation
        input_images: Union[PIL.Image.Image,
                            torch.Tensor,
        ...
        pixel_values = None
        input_ids = None
        if "llava" in self.cfg.original_architecture.lower():
            if isinstance(input, str) or isinstance(input, list) or \
                    (input_images is not None and isinstance(input, torch.Tensor)):
                input_processor = input
                if isinstance(input, torch.Tensor):
                    input_processor = self.tokenizer.decode(tokens[0])
                multimodal_input = self.processor(images=input_images,
                                                  text=input_processor,
                                                  return_tensors="pt")
                if "pixel_values" in multimodal_input:
                    pixel_values = multimodal_input["pixel_values"]

                if "input_ids" in multimodal_input:
                    input_ids = multimodal_input["input_ids"]

                attention_mask = multimodal_input["attention_mask"]
            else:
                attention_mask = utils.get_attention_mask(self.tokenizer,
                                                          tokens, prepend_bos)
                input_ids = tokens
        ...
        # not official implementation
        if "llava" in self.cfg.original_architecture.lower():
            embed = self.hook_embed(self.embed(input_ids,
                                               pixel_values=pixel_values))  # [batch, pos, d_model]
        else:
    ...
    def to_image_tokens(self,
                        input_images: Union[PIL.Image.Image,
                                            torch.Tensor,
                                            List[PIL.Image.Image],
                                            List[torch.Tensor]] = None,
                        ):
        return self.processor(images=input_images,
                              text="",
    ...
    def from_pretrained(
        ...
        **from_pretrained_kwargs,
        ...
        state_dict, vision_tower, multi_modal_projector = loading.get_pretrained_state_dict(
