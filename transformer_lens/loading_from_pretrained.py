...
from transformer_lens.pretrained.weight_conversions import (  # not official implementation
    ...
    convert_llama_weights, convert_llava_weights, convert_mingpt_weights,
    convert_mistral_weights, convert_mixtral_weights,
    convert_neel_solu_old_weights, convert_neo_weights, convert_neox_weights,
    convert_opt_weights, convert_phi3_weights, convert_phi_weights,
    convert_qwen2_weights, convert_qwen_weights, convert_t5_weights)
from transformers import (AutoConfig,  # not official implementation
...
def convert_hf_model_config(model_name: str, **kwargs):
    ...
    # not official implementation
        ...
        cfg_dict = {
            "d_model": hf_config.text_config.hidden_size,
            "d_head": hf_config.text_config.hidden_size // hf_config.text_config.num_attention_heads,
            "n_heads": hf_config.text_config.num_attention_heads,
            "d_mlp": hf_config.text_config.intermediate_size,
            "n_layers": hf_config.text_config.num_hidden_layers,
            "n_ctx": hf_config.text_config.max_position_embeddings,
            "eps": hf_config.text_config.rms_norm_eps,
            "d_vocab": hf_config.text_config.vocab_size,
            "act_fn": hf_config.text_config.hidden_act,
            "n_key_value_heads": (
                hf_config.text_config.num_key_value_heads
                if hf_config.text_config.num_key_value_heads != hf_config.text_config.num_attention_heads
                else None
            ),
            # This is done because the current implementation of GQA will use Grouped-Query Attention if
            # n_key_value_heads is not None, but hf_config.num_key_value_heads is sometimes specified as
            ...
            "positional_embedding_type": "rotary",
            "rotary_adjacent_pairs": False,
            "rotary_dim": hf_config.text_config.hidden_size // hf_config.text_config.num_attention_heads,
            "final_rms": True,
            "gated_mlp": True,
...
def get_pretrained_state_dict(
    ...
    dtype: torch.dtype = torch.float32,
    **kwargs,
) -> (Dict[str, torch.Tensor], torch.nn.Module, torch.nn.Module):
    """
    ...
    if (
        ...
        # not official implementation
            ...
            state_dict = convert_llava_weights(hf_model, cfg)
        else:
            raise ValueError(
        ...
        if "llava" in cfg.original_architecture.lower():
            vision_tower = hf_model.vision_tower
            multi_modal_projector = hf_model.multi_modal_projector

        return state_dict, vision_tower, multi_modal_projector
